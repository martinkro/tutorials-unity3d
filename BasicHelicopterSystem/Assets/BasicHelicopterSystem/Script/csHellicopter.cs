...
public class csHellicopter : MonoBehaviour {
    ...
    void FixedUpdate()
    {
        if (EngineTurnOnOff)
        {
            rigidbody.AddRelativeForce(Vector3.up * UpDownValue * UpDownVelocity);
    ...
    void MotorVelocityContorl()
    {
        if (EngineTurnOnOff)
        {
            float Hovering = (Mathf.Abs(rigidbody.mass * Physics.gravity.y) / UpDownValue); //for maintain altitude.
