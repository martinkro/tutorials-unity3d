...
public class bowAndArrow : MonoBehaviour {
		...
		case GameStates.game:
			...
			if (Input.GetMouseButtonUp (0) && arrowPrepared) {
				...
				if (!stringReleaseSoundPlayed) {
					audio.PlayOneShot(stringRelease);
					stringReleaseSoundPlayed = true;
				}
				// play arrow sound
				if (!arrowSwooshSoundPlayed) {
					audio.PlayOneShot(arrowSwoosh);
	...
	public void shootArrow() {
		if (arrow.GetComponent<Rigidbody>() == null) {
			...
			arrow.rigidbody.AddForce (Quaternion.Euler (new Vector3(transform.rotation.eulerAngles.x,transform.rotation.eulerAngles.y,transform.rotation.eulerAngles.z))*new Vector3(25f*length,0,0), ForceMode.VelocityChange);
		}
		arrowPrepared = false;
		stringPullout = stringRestPosition;
